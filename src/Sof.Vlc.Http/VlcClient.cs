...
namespace Sof.Vlc.Http
{
	...
	/// </summary>
	public sealed class VlcClient : VlcPollable
	{
		/// <summary>
		...
		public VlcClient(IPEndPoint ipEndPoint, string password, bool startPolling = true, TimeSpan pollingInterval = default(TimeSpan))
		...
		public VlcClient(string host, int port, string password, bool startPolling = true, TimeSpan pollingInterval = default(TimeSpan))
		{
			Host = host;
			Port = port;
		...
		/// </summary>
		public async Task Play() { await SendCommand("command=pl_play"); }

		/// <summary>
		/// Play the specified playlist item by it's Id.
		/// </summary>
		/// <param name="id">Playlist item Id.</param>
		public async Task Play(int id) { await SendCommand("command=pl_play&id=" + id); }

		/// <summary>
		/// Toggle play/pause. If currently stopped, play current item or first item in the playlist.
		/// </summary>
		public async Task Pause() { await SendCommand("command=pl_pause"); }

		/// <summary>
		/// Toggle play/pause. If currently stopped, play specified item.
		/// </summary>
		public async Task Pause(int id) { await SendCommand("command=pl_pause&id=" + id); }

		/// <summary>
		/// Pause the current media item.
		/// </summary>
		public async Task ForcePause() { await SendCommand("command=pl_forcepause"); }

		/// <summary>
		/// Play the current media item.
		/// </summary>
		public async Task ForceResume() { await SendCommand("command=pl_forceresume"); }

		/// <summary>
		/// Stop the current media item.
		/// </summary>
		public async Task Stop() { await SendCommand("command=pl_stop"); }

		/// <summary>
		/// Skip to the next media item in the playlist.
		/// </summary>
		public async Task Next() { await SendCommand("command=pl_next"); }

		/// <summary>
		/// Skip to the previous media item in the playlist.
		/// </summary>
		public async Task Previous() { await SendCommand("command=pl_previous"); }

		/// <summary>
		/// Delete the specified item from the playlist.
		/// </summary>
		/// <param name="id">Item's Id.</param>
		public async Task Delete(int id) { await SendCommand("command=pl_delete&id=" + id); }

		/// <summary>
		/// Clear the entire playlist.
		/// </summary>
		public async Task Clear() { await SendCommand("command=pl_empty"); }

		/// <summary>
		/// Toggle whether the playlist will be randomized.
		/// </summary>
		public async Task ToggleRandom() { await SendCommand("command=pl_random"); }

		/// <summary>
		/// Toggle whether the playlist will loop.
		/// </summary>
		public async Task ToggleLoop() { await SendCommand("command=pl_loop"); }

		/// <summary>
		/// Toggle whether the current playback item will be repeated.
		/// </summary>
		public async Task ToggleRepeat() { await SendCommand("command=pl_repeat"); }

		/// <summary>
		/// Toggle fullscreen mode.
		/// </summary>
		public async Task ToggleFullscreen() { await SendCommand("command=fullscreen"); }

		/// <summary>
		/// Take a snapshot of the currently playing video.
		/// </summary>
		public async Task TakeSnapshot() { await SendCommand("command=snapshot"); }

		/// <summary>
		/// Set the player's absolute volume (between 0-300)
		/// </summary>
		public async Task SetVolume(int val) { await SendCommand("command=volume&val=" + val); }

		/// <summary>
		/// Change the player's relative volume (the absolute volume is limited to between 0-300).
		/// </summary>
		public async Task SetRelativeVolume(int relVal) { await SendCommand("command=volume&val=" + (relVal > 0 ? "%2B" : "") + relVal); }

		/// <summary>
		/// Set the player's absolute volume as a percentage (between 0-100%)
		/// </summary>
		public async Task SetPercentageVolume(int val) { await SendCommand("command=volume&val=" + val + "%"); }
		...
		/// </summary>
		public async Task SetRelativePercentageVolume(int relVal) { await SendCommand("command=volume&val=" + (relVal > 0 ? "%2B" : "") + relVal + "%"); }
		...
		public async Task SortPlaylist(VlcPlaylistSortKey key, VlcPlaylistSortOrder order = VlcPlaylistSortOrder.Ascending) { await SendCommand("command=pl_sort&val=" + GetSortKeyString(key) + "&id=" + (int)order); }

		/// <summary>
		/// Sorts the playlist by the specified key in a descending order.
		/// </summary>
		/// <param name="key">Key to sort by.</param>
		public async Task SortPlaylistDescending(VlcPlaylistSortKey key) { await SendCommand("command=pl_sort&val=" + GetSortKeyString(key) + "&id=1"); }

		/// <summary>
		/// Set the playback position in seconds.
		/// </summary>
		public async Task SetPosition(int secs) { await SendCommand("command=seek&val=" + secs); }

		/// <summary>
		/// Changes the playback position relative to it's current position in seconds.
		/// </summary>
		public async Task SetRelativePosition(int relSecs) { await SendCommand("command=seek&val=" + (relSecs > 0 ? "%2B" : "") + relSecs); }

		/// <summary>
		/// Set the playback position as a percentage of the total length.
		/// </summary>
		public async Task SetPercentagePosition(int secs) { await SendCommand("command=seek&val=" + secs + "%"); }

		/// <summary>
		/// Changes the playback position as a percentage of the total length relative to it's current position.
		/// </summary>
		public async Task SetRelativePercentagePosition(int relSecs) { await SendCommand("command=seek&val=" + (relSecs > 0 ? "%2B" : "") + relSecs + "%"); }

		/// <summary>
		/// Set the playback rate e.g. 0.5 = half speed, 2.0 = double speed.
		/// </summary>
		public async Task SetRate(double val) { await SendCommand("command=rate&val=" + val); }

		/// <summary>
		/// Sets the audio delay in seconds (can be negative).
		/// </summary>
		public async Task SetAudioDelay(double secs) { await SendCommand("command=audiodelay&val=" + secs); }

		/// <summary>
		/// Sets the subtitle delay in seconds (can be negative).
		/// </summary>
		public async Task SetSubDelay(double secs) { await SendCommand("command=subdelay&val=" + secs); }

		/// <summary>
		/// Set the aspect ratio for the current playback media.
		/// </summary>
		public async Task SetAspectRatio(VlcAspectRatio ratio) { await SendCommand("command=aspectratio&val=" + GetAspectRatioString(ratio)); }

		/// <summary>
		/// Set the pre amp volume, in decibels (between -20 and +20).
		/// </summary>
		public async Task SetPreAmp(int db) { await SendCommand("command=preamp&val=" + Math.Max(-20, Math.Min(20, db))); }

		/// <summary>
		/// Enable the audio equalizer.
		/// </summary>
		public async Task EnableEqualizer() { await SendCommand("command=enableeq"); }

		/// <summary>
		/// Disable the audio equalizer.
		/// </summary>
		public async Task DisableEqualizer() { await SendCommand("command=enableeq&val=0"); }

		/// <summary>
		/// Set the equalizer preset, a list of which is given in the status response when the equalizer is enabled.
		/// </summary>
		public async Task SetEqualizerPreset(string val) { await SendCommand("command=setpreset&val=" + val); }

		/// <summary>
		/// Set the equalizer value for the specified band.
		/// </summary>
		public async Task SetEqualizerBandValue(int band, double val) { await SendCommand("command=equalizer&band=" + band + "&val=" + val); }

		/// <summary>
		/// Set the audio track using the stream id which can be found in the list of streams in the status response.
		/// </summary>
		public async Task SetAudioTrack(int streamId) { await SendCommand("command=audio_track&val=" + streamId); }

		/// <summary>
		/// Set the video track using the stream id which can be found in the list of streams in the status response.
		/// </summary>
		public async Task SetVideoTrack(int streamId) { await SendCommand("command=video_track&val=" + streamId); }

		/// <summary>
		/// Set the subtitle track using the stream id which can be found in the list of streams in the status response.
		/// </summary>
		public async Task SetSubtitleTrack(int streamId) { await SendCommand("command=subtitle_track&val=" + streamId); }

		/// <summary>
		/// Add a subtitle track to the current media item.
		/// </summary>
		public async Task AddSubtitleTrack(string path) { await SendCommand("command=addsubtitle&val=" + Uri.EscapeDataString(path)); }
		...
		/// </summary>
		public async Task PlayTitle(int num) { await SendCommand("command=title&val=" + num); }

		/// <summary>
		/// Jump to the specified chapter for the current playback item.
		/// </summary>
		public async Task PlayChapter(int num) { await SendCommand("command=chapter&val=" + num); }

		/// <summary>
		/// Request an updated status for the media player and current playback item.
		/// </summary>
		public async Task UpdateStatus() { await SendCommand(); }
